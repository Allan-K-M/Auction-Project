...
public class ManageAuctionsState extends State {
    ...
    private State seeAuctions() {
        ...
        allUserAuctions.stream().forEach(auction -> {
            ...
            allAuctionsBids.stream().forEach(bid -> {
    ...
    private State closeAuction() {
        try {
            ...
            display("============================");
            display("Owner Id || Price || Quantity ");
            winningBids.stream().forEach(bid -> {
    ...
    private State getWonBids() {
        List<Bid> wonBids = MenuData.AUCTION_MANAGEMENT.getUserWonBids(MenuData.currentUser.getUserName());
        display("================================");
        if (wonBids.isEmpty()) {
            display("You have no won Bids! ");
        } else {
            display("Auction Id || Quantity || Price");
            wonBids.stream().forEach(bid -> display(bid.getAuctionId() + "  ||  " + bid.getQuantity() + " ||  " + bid.getValue()));
        }
        display("================================");
    ...
    private State getLostBids() {
        List<Bid> lostBids = MenuData.AUCTION_MANAGEMENT.getUserLostBids(MenuData.currentUser.getUserName());
        display("================================");
        if (lostBids.isEmpty()) {
            display("You have no won Bids! ");
        } else {
            display("Auction Id || Quantity || Price");
            lostBids.stream().forEach(bid -> display(bid.getAuctionId() + "  ||  " + bid.getQuantity() + " ||  " + bid.getValue()));
        }
        display("================================");
        return MenuData.manageAuctionsState.start();
    }
